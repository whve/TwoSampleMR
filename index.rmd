...
Outcome names in the output of mr() combine the original trait name and outcome trait ID. 

```{r }
head(res)
```

These components can be split into separate columns:
...
Sometimes it is useful to combine all results and study level characterists into a single file. This can be done using:

```
combine_all_mrresults(res,het,pleiotropy,res_single)
```

This combines all results from mr(), mr_heterogeneity(), mr_pleiotropy() and mr_singlesnp() into a single data frame. It also merges the results with outcome study level characteristics from the available_outcomes() function. If desired, it also exponentiates results (e.g. if the user wants log odds ratio converted into odds ratios with 95 percent confidence intervals). 
